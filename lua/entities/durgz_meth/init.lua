...
ENT.LASTINGEFFECT = 30; --how long the high lasts in seconds

function ENT:High(activator,caller)

	sound.Play("drugs/insufflation.wav", activator:GetPos(), 75, 100, 1)
	self:Say(activator, "WHOO! YEAH!! FUCK YEAH! I FEEL GREAT!! I FEEL ALIVE!!")	
...
function ENT:AfterHigh(activator, caller)
	if( activator:Health() == 1 ) or ( activator:Health() == 2 ) then
...
hook.Add("Think", "durgz_meth_resetspeed", function()
    for id,pl in pairs(player.GetAll())do
        if pl:GetNetworkedFloat("durgz_meth_high_end") < CurTime() then
			pl:SetGravity(1)
        end
    end
end)